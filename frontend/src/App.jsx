...
CRITICAL: Reply ONLY in ${langName}. Max 2-3 sentences. Simple language. Be specific and encouraging.`
  ...
  const handleDownload = async () => {
    try {
      // Extract platform from sub string (e.g., "Swiggy Delivery Partner · Bengaluru · Member since Jan 2024" -> "Swiggy")
      const platform = p.sub.split(' · ')[0].split(' ')[0]

      // Clean avgIncome (remove ₹ and commas)
      const avgIncome = p.stats[0].value.replace('₹', '').replace(/,/g, '')

      const workerData = {
        name: p.name,
        platform: platform,
        avgIncome: avgIncome,
        transactions: p.stats[1].value,
        incomeVariance: p.stats[3].value,
        topPayer: p.stats[2].value,
        tips: p.tips,
        months: p.months,
        incomes: p.income,
      }
    ...
    <div style={{ minHeight: '100vh', background: '#FFFBF7', color: '#1A1410', fontFamily: 'Sora, system-ui, sans-serif' }}>
      ...
      <main style={{ maxWidth: 960, margin: '0 auto', padding: '28px 20px 48px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ ...card, padding: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleVoice} style={{
              ...
              background: listening ? '#DC2626' : '#FEF3E2',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10 }}>
          <button onClick={handleDownload} style={{ background: '#F97316', color: 'white', border: 'none', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, padding: '13px 20px', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>{L.download}</button>
