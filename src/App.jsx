...
const UI = {
  ...
  te: {
    score:'క్రెడిట్ స్కోర్', breakdown:'స్కోర్ బ్రేక్‌డౌన్', chart:'మాసిక ఆదాయం (₹)',
    tips:'AI ఆర్థిక సూచనలు', l1:'సగటు మాసిక ఆదాయం', l2:'ఒక నెలలో లావాదేవీలు',
    l3:'ప్రముఖ చెల్లింపుదారు', l4:'ఆదాయ వ్యత్యాసం', loan:'ద్రవ్యత అర్హత',
    journey:'స్కోర్ ప్రయాణం — 6 నెలలు', ago:'6 నెలల క్రితం', today:'ఈ రోజు',
    chatTitle:'Kharcha సహాయకుడు', chatSub:'గిగ్ వర్కర్లకు ఆర్థిక సలహాలు',
    placeholder:'మీ స్కోర్, రుణం, సూచనలు అడగండి...', download:'📄 రిపోర్ట్ డౌన్‌లోడ్',
    whatsapp:'WhatsAppలో పంచుకోండి', lender:'లెండర్ పోర్టల్ ↗', why:'ఈ స్కోర్ ఎందుకు? ↗',
    source:' సోర్స్: UPI లావాదేవీ చరిత్ర · సెలరీ స్లిప్ అవసరం లేదు',
  },
}
...
const GREETINGS = {
  ...
  te: {
    ravi:   "నమస్కారం రవి! మీ స్కోర్ 684 — మంచి స్థితి. మీరు ₹50,000 వరకు మైక్రోలాన్‌కు అర్హులు.",
    meena:  "నమస్కారం మీనా! స్కోర్ 521. ఆదాయ స్థిరత్వం మీ ప్రధాన అవకాశంగా ఉంది.",
    suresh: "స్వాగతం సురేశ్! స్కోర్ 742 — చాలా బాగుంది. మీరు టాప్ 15% లో ఉంటారు!",
  },
}

// Additional UI strings used by sign-in
UI.en.signinTitle = 'Sign in to Kharcha'
UI.en.signinPhonePlaceholder = 'Mobile number (10 digits)'
UI.en.signinPinPlaceholder = '4-digit PIN'
UI.en.signinButton = 'Sign in'
UI.en.signinInvalid = 'Invalid demo login — try 777 / 888 / 999'

UI.kn.signinTitle = 'Kharcha ಗೆ ಸೈನ್ ಇನ್'
UI.kn.signinPhonePlaceholder = 'ಮೊಬೈಲ್ ಸಂಖ್ಯೆ (10 ಅಂಕಿಗಳು)'
UI.kn.signinPinPlaceholder = '4-ಅಂಕಿ ಪಿನ್'
UI.kn.signinButton = 'ಲಾಗಿನ್'
UI.kn.signinInvalid = 'ತಪ್ಪಾದ ಡೆಮೋ ಲಾಗಿನ್ — 777 / 888 / 999 ಪ್ರಯತ್ನಿಸಿ'

UI.hi.signinTitle = 'Kharcha में साइन इन'
UI.hi.signinPhonePlaceholder = 'मोबाइल नंबर (10 अंक)'
UI.hi.signinPinPlaceholder = '4-अंकों का PIN'
UI.hi.signinButton = 'साइन इन'
UI.hi.signinInvalid = 'अमान्य डेमो लॉगिन — 777 / 888 / 999 आज़माएं'

UI.ta.signinTitle = 'Kharcha இல் சமிக்ஞை'
UI.ta.signinPhonePlaceholder = 'மொபைல் எண் (10 இலக்கங்கள்)'
UI.ta.signinPinPlaceholder = '4 இலக்க பின்'
UI.ta.signinButton = 'அன்புடன் நுழைக'
UI.ta.signinInvalid = 'தவறான டெமோ புகுபதிகை — 777 / 888 / 999 முயற்சிக்கவும்'

UI.te.signinTitle = 'Kharcha లో సైన్ ఇన్'
UI.te.signinPhonePlaceholder = 'మొబైల్ నంబర్ (10 అంకెలు)'
UI.te.signinPinPlaceholder = '4-అంకెల PIN'
UI.te.signinButton = 'సైన్ ఇన్'
UI.te.signinInvalid = 'చెల్లని డెమో లాగిన్ — 777 / 888 / 999 తీసుకువచ్చి చూడండి'
...
CRITICAL: Reply ONLY in ${langName}. Max 2-3 sentences. Simple language. Be specific and encouraging.`
  ...
  const demoMap = { '7777777777':'suresh', '8888888888':'ravi', '9999999999':'meena' }

  const handleSignIn = (e) => {
    e?.preventDefault?.()
    setSignInError('')
    const cleaned = (phone||'').replace(/\D/g,'')
    const target = demoMap[cleaned]
    if (!target) {
      setSignInError(UI[lang]?.signinInvalid || UI.en.signinInvalid)
      return
    }
    // success: set logged in persona and make it active
    setLoggedInPersona(target)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FFFBF7', color: '#1A1410', fontFamily: "Sora, 'Noto Sans Kannada', 'Noto Sans Telugu', system-ui, sans-serif" }}>

      {/* TOP BAR */}
      <header style={{ background: '#1A1410', position: 'relative', zIndex: 50 }}>
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', height: 52 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: '#F97316', letterSpacing: '-0.5px' }}>
            Kharcha<span style={{ color: 'white' }}>.</span>
          </div>
          <div style={{ display: 'flex', gap: 6, background: 'rgba(255,255,255,0.07)', borderRadius: 99, padding: 4, alignItems: 'center' }}>
            {!loggedInPersona && Object.keys(personas).map(key => (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {loggedInPersona ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ fontSize: 13, color: 'white', fontWeight: 700 }}>{personas[loggedInPersona].name}</div>
                <button onClick={() => { setLoggedInPersona(null); setActivePersona('ravi') }} style={{ width: 36, height: 36, borderRadius: 18, background: '#F97316', border: 'none', color: 'white', fontWeight: 800 }}>{personas[loggedInPersona].initial}</button>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button onClick={() => setShowSignIn(s => !s)} style={{ background: 'transparent', color: 'white', border: '1px solid rgba(255,255,255,0.12)', padding: '6px 12px', borderRadius: 8, cursor: 'pointer' }}>{UI[lang]?.signinButton || UI.en.signinButton}</button>
              </div>
            )}
          </div>
        </div>

        {showSignIn && !loggedInPersona && (
          <div style={{ maxWidth: 960, margin: '6px auto', padding: 14, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: 12, color: 'white' }}>
            <form onSubmit={handleSignIn} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>{UI[lang]?.signinTitle || UI.en.signinTitle}</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <input value={phone} onChange={e => setPhone(e.target.value)} placeholder={UI[lang]?.signinPhonePlaceholder || UI.en.signinPhonePlaceholder} style={{ padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: 'white', color: '#1A1410' }} />
                  <input value={pin} onChange={e => setPin(e.target.value)} placeholder={UI[lang]?.signinPinPlaceholder || UI.en.signinPinPlaceholder} style={{ padding: '8px 10px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', background: 'white', color: '#1A1410', width: 110 }} />
                  <button type="submit" style={{ background: '#F97316', color: 'white', border: 'none', padding: '8px 12px', borderRadius: 8, fontWeight: 700 }}>{UI[lang]?.signinButton || UI.en.signinButton}</button>
                </div>
                {signInError && <div style={{ color: '#FEE2E2', marginTop: 8 }}>{signInError}</div>}
              </div>
            </form>
          </div>
        )}
